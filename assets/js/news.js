(() => {
  ...
  const renderList = async () => {
    ...
    const { manifest, lang = "en", basePath = "/en/news/", postPath = "/en/news/post/" } =
    ...
    try {
      ...
      if (!pageItems.length) {
        ...
        empty.textContent = "No published news posts yet.";
      ...
      listEl.innerHTML = "<p>Unable to load news right now.</p>";
  ...
  const renderDetail = async () => {
    ...
    const { manifest, lang = "en", basePath = "/en/news/" } = document.body?.dataset || {};
    const locale = lang === "en" ? "en-US" : lang;
    ...
    try {
      ...
      if (!post) {
        container.innerHTML = `<p>Post not found. <a href="${basePath}">Back to News</a></p>`;
        return;
      }
